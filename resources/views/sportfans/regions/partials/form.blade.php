<div class="form-group">
	<strong>{{Form::label('id', 'ID.')}}</strong>
	{{Form::text('id',null,['class'=>'form-control','id'=>'id','readonly'=>'readonly'])}}
</div>
<div class="form-group">
	<strong>{{Form::label('caption', 'Nombre')}}</strong>
	{{Form::text('caption',null,['class'=>'form-control','id'=>'caption','onchange'=>"strToSlug('caption','slug')"])}}
</div>
<div class="form-group">
	<strong>{{Form::label('slug', 'Slug')}}</strong>
	{{Form::text('slug',null,['class'=>'form-control','id'=>'slug','readonly'=>'readonly'])}}
</div>
<div class="form-group">
	<strong>{{Form::label('external_link', 'Enlace Externo')}}</strong>
	{{Form::text('external_link',null,['class'=>'form-control','id'=>'external_link'])}}
</div>
<div class="form-group">
	<strong>{{Form::label('parent_id', 'Región Superior')}}</strong>
	{{Form::select('parent_id',[''=>'---',$parentlist],null,['class'=>'form-control','id'=>'parent_id'])}}
</div>
<div class="form-group">
	<strong>{{Form::label('abstract', 'Resumen')}}</strong>
	{{Form::textarea('abstract',null,['class'=>'form-control','id'=>'abstract','rows'=>'2'])}}
</div>
<div class="form-group mb-3">
	<strong>{{Form::label('formFile', 'Bandera o Referencia',['class'=>'form-label'])}}</strong>
	@if($region->picture)
		<img src="{{env('APP_URL')}}:8000/storage/regions/{{$region->picture}}" class="img-thumbnail rounded" height="350px" width="350px">
	@endif
	{{Form::file('picture',['class'=>'form-control'],null)}}
</div>
<hr/>
<div class="form-group">
	{{Form::submit('Guardar',['class'=>'btn btn-sm btn-primary'])}}
	<a href="{{route('regions.index')}}" class="btn btn-sm btn-secondary">
            volver
    </a>
</div>