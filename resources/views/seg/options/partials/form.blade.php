			...
			<label>
				<div class="col">
					...
					@if(isset($option) && $option->profiles->contains($profile->id))
						{{ Form::checkbox('profiles[]', $profile->id,true,['class'=>'form-check-input'] ) }} {{$profile->caption}}
					@else
						{{ Form::checkbox('profiles[]', $profile->id,false,['class'=>'form-check-input'] ) }} {{$profile->caption}}
					@endif
					</div>
				</div>
			</label>
